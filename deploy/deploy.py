...
def _api(region: str) -> str:
    host = "eu.pythonanywhere.com" if region == "eu" else "www.pythonanywhere.com"
...
def get_or_create_webapp(base: str, headers: dict, username: str, domain: str, python_version: str) -> dict:
    """Create the web app if it doesn't exist yet; return its config."""
    list_resp = requests.get(f"{base}/user/{username}/webapps/", headers=headers)
    ...
    print(f"  Creating web app '{domain}' with Python {python_version}...")
    create_resp = requests.post(
        f"{base}/user/{username}/webapps/",
        headers=headers,
        data={"domain_name": domain, "python_version": f"python{python_version.replace('.', '')}"},
    )
...
def set_virtualenv(base: str, headers: dict, username: str, domain: str) -> None:
...
def reload_webapp(base: str, headers: dict, username: str, domain: str) -> None:
...
def main() -> None:
    ...
    parser.add_argument("--token", required=True, help="Your PythonAnywhere API token")
    parser.add_argument("--region", choices=["us", "eu"], default="us",
                        help="Server region: 'us' (default) or 'eu'")
    parser.add_argument("--python", default="3.12", metavar="VERSION",
